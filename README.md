...
两个刻意的设计：**停管理器是写一个 `logs\stop.req` 让它优雅退出**（直接杀进程会跳过还原逻辑，电话簿会留在停放态）；**关窗口只是收进托盘**（管理器、重拨模式都继续跑）—— 真要退出走**托盘右键 →「退出」**，或者界面最下面那排的**「退出」按钮**；退出时如果还有模式在跑，只问一句：**「是」= 退出并把它们一起停掉 /「否」= 什么都不做**。
