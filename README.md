...
| 模式 | 背后 | 说明 |
| --- | --- | --- |
| **正常重拨模式** | `Redial-UntilCampusReady.ps1` | 一直换出口直到确认好出口。运行方式二选一：**普通重拨** / **高速模式**（`-HighBandwidthMode`，重拨到西电测速超 150 Mbps）。（`-TestOnly`「只测当前出口」从 GUI 撤了 —— 测当前出口用下面的「测试出口」按钮；命令行的 `-TestOnly` 仍在。）输出**实时**进日志区，完整输出另存 `logs\redial-*.out.txt`。<br>普通重拨默认勾着**「拿到好出口后继续盯着」**：给脚本喂一串 `Y`，等于替它一直按那个「要不要继续测速或者重新拨号」的询问 —— 不退出、隔一轮再探一次、探到坏的就自动重新拨号。取消勾选就回到「拿到好出口即收工」的一次性行为 |
| **游戏模式** | `Switch-NetworkPath.ps1`（常驻管理器） | 停放 → 确认好出口转主用 → 变坏立刻退回 Wi-Fi。硬前提是**先连上热点**，没连上按钮置灰并提示；面板上还有它自己的**开机自启** |

共用按钮：**恢复拨号优先**（等价 `-RestoreDialPriority`）、**测试出口**（两个出口各测一轮：2 个探针 + 出口源地址确认 + **各 5 秒网速**，结果含 Mbps 追加到 `logs\exit-compare.log`；**游戏模式或重拨模式在跑时该按钮置灰** —— 它们会污染这一轮的数字）、**断开拨号**（同一个按钮，拨号连着就断开、断开着就拨回来；不碰电话簿开关，所以拨回来仍然是拨号优先）。

状态面板每秒读一次 `logs\status.json`（纯文件读，不会一直起新进程）：当前承载、健康已持续多久、电话簿开关、**拨号是否真的握着默认路由**、Wi-Fi 保底是否可用。

日志区带颜色：**重点**（模式开停与结果、「测试出口」的结果、失败/警告）**橙色**，**普通**（重拨探测表流、`正在…` 过程提示、界面诊断）**黑色**；超过 12 万字符从头滚动，盯着跑一整晚也不会越占越多。

两个刻意的设计：**停管理器是写一个 `logs\stop.req` 让它优雅退出**（直接杀进程会跳过还原逻辑，电话簿会留在停放态）；**点 X 不再是"关掉就完事"** —— **只要游戏模式或重拨模式还在跑，关不掉**（会弹一句提示，系统菜单里的「关闭」也置灰），**没有任何模式在跑时点 X 才正常关闭**。真要退出（尤其模式还在跑时）用最下面的**「退出」按钮**：**「是」= 退出并把它们一起停掉 /「否」= 什么都不做**。（托盘图标那一套已按要求移除 —— 没有托盘右键了。）
