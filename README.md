...
- **健康检查**：主用状态下**每 3 秒**探测一轮，每轮 3 个探针，**通过 ≥ 2 个（多数）就算健康**，否则判不健康并**立刻切回 Wi-Fi**（`-FailThreshold` 默认 1，不再要求连续失败两次）。
  **一旦这一轮已经不可能达标就立刻停手**，不再等剩下的探针超时 —— 探针按 `abvolcapi / apiv2 / abvolcapi` 轮转，所以拆成 `-Count 2` + `-Count 1` 两段跑：前两个若都挂，第三个再好也只凑到 1 个，直接判死返回。覆盖的 canary 与一次跑 3 个完全相同，判据也一样，只是省掉一整个探测超时的等待。
- **健康检查用更短的探测超时**（`-HealthTimeoutSeconds` 默认 2 秒，晋升验证仍用 `-TimeoutSeconds`=4 秒）：实测健康出口的单个探针只要 **150–460ms**，2 秒已是 4–13 倍余量。于是坏出口的等待从 `3 × 4 = 12` 秒压到最多 `2 × 2 = 4` 秒。
...
- **健康检查怎么记日志**：健康时**不写日志文件**，只在控制台原地刷一行（不滚动）：
  ...
  ```text
  健康检查：未通过 —— 通过 1/3（阈值 2）（直连探测）。此前已健康 12分34秒。
...
| `-HealthPassThreshold` | 2 | 一轮里通过几个探针才算健康（3 个探针里的多数 = 2）|
