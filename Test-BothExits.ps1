...
function Test-OneExit {
    ...
    if ($targets.Count -eq 0) {
        return [pscustomobject]@{ Name = $Name; Error = 'canary 一个 IPv4 都解析不出来'; ExpectIp = $ExpectIp; Passed = -1; Total = -1; Detail = ''; Egress = ''; Ok = $false }
    }
    ...
    if ($installed.Count -ne $targets.Count) {
        ...
        return [pscustomobject]@{ Name = $Name; Error = '引导路由没装全'; ExpectIp = $ExpectIp; Passed = -1; Total = -1; Detail = ''; Egress = ''; Ok = $false }
    }
...
function Format-ExitLine {
    ...
    return ('  {0,-14} {1} {2}/{3}   {4}   出口源地址 {5}' -f $Tag, $verdict, $Result.Passed, $Result.Total, $Result.Detail, $egressNote)
}
...
while ($true) {
    ...
    Write-Line ''
    Write-Line ('──────────────────────────────────────────────────────────────')
    Write-Line ("[{0}] 第 {1} 次" -f (Get-Date -Format 'HH:mm:ss'), $round) 'Cyan'

    # 拨号出口（每轮都重新解析接口，重拨后 ifIndex 会变）
    ...
    if ($dialIf) {
        ...
        Write-Line (Format-ExitLine -Result $dialResult -Tag ('拨号 {0}' -f $dialIp)) $(if ($dialResult.Ok) { 'Green' } else { 'Red' })
    }
    else {
        Write-Line ('  拨号          未连接（先 rasdial {0} 再测）' -f $DialName) 'Yellow'
        $dialResult = $null
    }
    ...
    if ($wifiAdapter -and $wifiAdapter.Status -eq 'Up') {
        ...
        if ($wifiRoute -and $wifiRoute.NextHop -ne '0.0.0.0') {
            ...
            Write-Line (Format-ExitLine -Result $wifiResult -Tag ('Wi-Fi {0}' -f $wifiIp)) $(if ($wifiResult.Ok) { 'Green' } else { 'Red' })
        }
        else {
            Write-Line '  Wi-Fi         没有默认路由（网卡连着但没拿到网关）' 'Yellow'
        }
    }
    else {
        Write-Line '  Wi-Fi         网卡不可用' 'Yellow'
    }

    # 对比结论
    $dOk = ($dialResult -and $dialResult.Ok)
    $wOk = ($wifiResult -and $wifiResult.Ok)
    $verdict = if ($dOk -and $wOk) { '两个出口都可用' }
    elseif ($dOk) { '只有拨号可用' }
    elseif ($wOk) { '只有 Wi-Fi 可用' }
    else { '两个出口都不可用' }
    Write-Line ("  => {0}" -f $verdict) $(if ($dOk -or $wOk) { 'Gray' } else { 'Red' })

    # 追加日志
    foreach ($pair in @(@('pppoe', $dialResult, $dialIp), @('wifi', $wifiResult, $wifiIp))) {
        $tag = $pair[0]; $res = $pair[1]; $ip = $pair[2]
        $line = if ($res) {
            '{0}  {1,-6} {2,-16} 通过 {3}/{4}  {5}  源={6}' -f $stamp, $tag, $ip, $res.Passed, $res.Total, $res.Detail, $res.Egress
        }
        else {
...
$left = @(Get-NetRoute -AddressFamily IPv4 -ErrorAction SilentlyContinue | Where-Object { $_.DestinationPrefix -like '*/32' -and $_.DestinationPrefix -notlike '10.*' -and $_.DestinationPrefix -notlike '172.29*' -and $_.DestinationPrefix -ne '255.255.255.255/32' })
if ($left.Count) { $left | ForEach-Object { Write-Line ('  残留 {0} ifIndex={1}' -f $_.DestinationPrefix, $_.InterfaceIndex) 'Red' } }
else { Write-Line '  无 ✓' 'Green' }
